...
__global__ void rungekutta_step2_smem(complex *a0, complex *a1, complex *a2){
  ...
  if(i<nx && j<ny) smem[it] = ELEM_LOAD<usetex, 1, Real>(a1, id);

	if(i>0 && j>0 && i<nx-1 && j<ny-1){
		if(threadIdx.x == 0) smem[it-1] = ELEM_LOAD<usetex, 1, Real>(a1, id-1);
		if(threadIdx.y == 0) smem[index(0,-1)] = ELEM_LOAD<usetex, 1, Real>(a1, id-nx);
		if(threadIdx.x == blockDim.x - 1) smem[it+1] = ELEM_LOAD<usetex, 1, Real>(a1, id+1);
		if(threadIdx.y == blockDim.y - 1) smem[index(0,1)] = ELEM_LOAD<usetex, 1, Real>(a1, id+nx);  
	}   
		__syncthreads();
	...
	if(i>0 && j>0 && i<nx-1 && j<ny-1){
		...
		a2[id] = seconddxy * 0.75 + smem[it] * 0.25 + res * (dt<Real>() * 0.25);
	}
}
...
template <class Real>
void RK2<Real>::callKernel(uint3 grid, uint3 block, size_t smem){
	switch(typ){
		case 0:
			rungekutta_step2<Real, false><<<grid, block, smem, 0>>>(a0, a1, a2);
		break;
		case 1:
			rungekutta_step2<Real, true><<<grid, block, smem, 0>>>(a0, a1, a2);
		break;
		case 2:
			rungekutta_step2_smem<Real, false><<<grid, block, smem, 0>>>(a0, a1, a2);
		break;
		case 3:
			rungekutta_step2_smem<Real, true><<<grid, block, smem, 0>>>(a0, a1, a2);
		break;
		default:
			rungekutta_step2<Real, false><<<grid, block, smem, 0>>>(a0, a1, a2);
		break;
