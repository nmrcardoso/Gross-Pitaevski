...
class TuneKey {
  ...
  protected:
    ...
    if (tuneGridDim()) {
      ...
      if(param.block.x > max_threads) printfError("Local volume is too large for device");
    }
  ...
  public:
    ...
    if (param.block.x > max_threads) printfError("volume is too large for device");
